...
SELECT 
    ...
    ROUND(SUM(SumInsured)) AS TotalSumInsured
FROM FarmersInsuranceData
...
-- 	Q6.	Retrieve the total number of farmers covered (TotalFarmersCovered) and the sum of premiums (GrossPremiumAmountToBePaid) 
-- 		for each state (srcStateName) where the insured land area (InsuredLandArea) is greater than 5.0 and the Year is 2018.
-- ###
-- 	[5 Marks]
-- ###
-- TYPE YOUR CODE BELOW >
...
SELECT 
    ...
    SUM(TotalFarmersCovered) AS TotalFarmersCovered
FROM FarmersInsuranceData
-- WHERE SumInsured > 500000  (there isn't any record more than 500000 so instead of 5,00,000 we used 50,000 as filter_
WHERE SumInsured > 50000
GROUP BY srcStateName
ORDER BY TotalFarmersCovered DESC;
...
FROM FarmersInsuranceData
ORDER BY srcStateName;
-- creating short name with only first 3 charcter will lead to duplicate
...
SELECT 
    srcStateName,
    srcDistrictName,
    MAX(srcYear) AS srcYear,
    MAX(TotalPopulation) AS TotalPopulation,
    MAX(FarmersPremiumAmount) AS HighestFarmersPremiumAmount
FROM FarmersInsuranceData
GROUP BY srcStateName, srcDistrictName
-- HAVING MAX(FarmersPremiumAmount) >= 200000000; (none of the district have highest premium amount more than 20 cr)
HAVING MAX(FarmersPremiumAmount) >= 2000 -- (as farmer premium amount are less, we are assuming the limit 2000 rather than 20 crores)
...
FROM FarmersInsuranceData
GROUP BY srcStateName, srcDistrictName
HAVING SUM(FarmersPremiumAmount) > 10000 -- ((as farmer premium amount are less, assuming the limit 10000 rather than 100 crores)
...
FROM FarmersInsuranceData
GROUP BY srcStateName, srcDistrictName
HAVING SumInsured > (	SELECT SumInsured 
						FROM FarmersInsuranceData
